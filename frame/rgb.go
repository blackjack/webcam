...
func newRGBFramer(w, h, r, g, b int) func([]byte, func()) (Frame, error) {
	...
	return func(buf []byte, rel func()) (Frame, error) {
		return frameRGB(size, bw, w, h, r, g, b, buf, rel)
	}
}

// Wrap a raw webcam frame in a Frame so that it can be used as an image.
func frameRGB(size, bw, w, h, rof, gof, bof int, b []byte, rel func()) (Frame, error) {
	if len(b) != size {
		if rel != nil {
			defer rel()
		}
