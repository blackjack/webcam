...
#define _ioctl(f,r,a) __ioctl(f,r,a);

int __ioctl( int fh, int request, void* arg )
{
  int r;
  do {
...
void* newFrmSizeEnum(uint32_t code) {
  const size_t size = sizeof(struct v4l2_frmsizeenum);
